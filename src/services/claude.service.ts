...
import { ParsedSpending, ParsedSpendingSchema, ExternalServiceError } from '../types';
...
export class ClaudeService {
  static async parseSpendingMessage(
    userMessage: string,
    userContext?: {
      recentSpending?: Array<{ amount: number; category: string; description?: string }>;
      timezone?: string;
    }
  ): Promise<ParsedSpending> {
    try {
      const systemPrompt = this.buildParsingSystemPrompt();
      const userPrompt = this.buildParsingUserPrompt(userMessage, userContext);

      logger.debug('Calling Claude API for spending parsing', {
      ...
      const validated = ParsedSpendingSchema.parse(parsed);

      logger.info('Successfully parsed spending message', {
      ...
      logger.error('Failed to parse spending message', {
        error: error.message,
        userMessage,
      });
  ...
  static async generateAcknowledgment(
    ...
    try {
      ...
      logger.error('Failed to generate acknowledgment', {
        error: error.message,
      });
  ...
  private static buildEngagingSystemPrompt(): string {
...
Now respond to the user's spending with YOUR personality.`;
  ...
  private static buildEngagingUserPrompt(
...
- They've spent ₦${context.weekTotal.toLocaleString()} total this week`;
  ...
  private static getFallbackResponse(
    spending: ParsedSpending,
    context: { todayTotal: number; weekTotal: number; categoryTotal?: number }
  ): string {
    ...
    if (amount > 10000) {
      responses.push(
        `₦${amount.toLocaleString()}? Okay o. You've spent ₦${context.weekTotal.toLocaleString()} this week.`,
        `Big man things. ₦${amount.toLocaleString()}. That's ₦${context.weekTotal.toLocaleString()} total this week.`
      );
    }
  ...
  private static buildParsingSystemPrompt(): string {
...
Respond ONLY with a JSON object, no other text or markdown.`;
  ...
  private static buildParsingUserPrompt(
    message: string,
    context?: {
      recentSpending?: Array<{ amount: number; category: string; description?: string }>;
      timezone?: string;
